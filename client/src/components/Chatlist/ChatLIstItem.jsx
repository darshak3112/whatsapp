...
function ChatLIstItem({ data, isConatctPage = false }) {
  const [{ userInfo, currentChatUser }, dispatch] = useStateProvider();
  const handleConatactList = () => {
    if (!isConatctPage) {
      dispatch({
        ...
        user: {
          ...
          email: data.email,
          id: userInfo.id === data.senderId ? data.recieverId : data.recieverId,
        },
      });
    } else {
      dispatch({
        ...
        <div className="flex border-b border-conversation-border pb-2 pt-1 pr-2 ">
          <div className="flex justify-between w-full ">
            <span className=" text-secondary line-clamp-1 text-sm">
              {isConatctPage ? (
                data?.about || "I am using Whatsapp"
              ) : (
